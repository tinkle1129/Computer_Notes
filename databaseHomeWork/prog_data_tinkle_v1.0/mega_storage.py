...
class MegaStorage(object):
	...
	def __init__(self, tablename):  # each table coresponds to one file with suffix .dat
		...
		if not os.path.exists('database/'+tablename + '.dat'):  # the data file does not exist
			print 'table file ' + tablename + '.dat does not exists'
			self.f_handle = open('database/'+tablename + '.dat', 'w+')  # text file
			self.f_handle.close()
			self.f_handle = open('database/'+tablename + '.dat', 'r+')
			print tablename + '.dat has been created'
			self.num_of_fields = None

		else:  # the file do exist

			self.f_handle = open('database/'+tablename + '.dat', 'r+')  ##a
			print 'table file ' + tablename + '.dat is opened now'
	...
	# --------------------------------
	# to insert a record into table, the values are input by users
	# input
	...
	def insert_record(self, field_name_list):
		...
		if len(field_name_list) > 0:  # schema is provided
			...
			for i in range(len(field_name_list)):
				temp_value = raw_input('pleas insert the field value of ' + field_name_list[i] + ' :')
